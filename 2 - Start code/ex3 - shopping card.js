...
function getCartTotalAmount() {
  ...
  for(i=0; i<SHOPPING_CART.length; i++){
    let sc = SHOPPING_CART[i];
    let product = PRODUCTS.find((p)=> sc.id === p.id);
    result += sc.quantity*product.price;
  }
  return result;
}
...
function addProductToCart(productId) {
  // Write your code here
  let shopC = SHOPPING_CART.find((sc)=> sc.id === productId);
   if(shopC){
    shopC.quantity++;
  } 
  else{
